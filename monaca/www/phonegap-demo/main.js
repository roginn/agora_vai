...
function upload_pic(data) {
    ...
    request.open('POST', 'http://agoravai.querobolsa.space/uploads/file', true);
    //request.setRequestHeader('Content-Type', 'application/octet-stream; charset=UTF-8');
    request.send(formdata);
    */
    jQuery('#spinner').show()
    jQuery.post(
        'http://agoravai.querobolsa.space/uploads/file',
        {
            data: data
        },
        function(response) {
...
function show_pic() {
    navigator.camera.getPicture(upload_pic, fail, {
        quality : 100,
